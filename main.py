...
from proto_process import (protol, gui_blackbox_data_queue,blackbox_data_len_dict)
...
class read_data_thread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            # TODO:
            data_send = ['03', '0']
            self.parent.protol.protol_pack_data(data_send)
            time.sleep(2)
    ...
    def read_bat_data(self):
        if 0 == self.read_data_start_flag:
            if 1 == self.uart_com_run_status:
                print("self.read_data_thread.start")
                self.read_data_start_flag = 1
                self.read_data_thread.start()
        else:
            print("self.read_data_thread.stop")
            self.read_data_start_flag = 0
